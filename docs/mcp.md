...
The Blaze CLI includes a built-in
[Model Context Protocol (MCP)](https://modelcontextprotocol.io) server. MCP is
an open standard that lets AI assistants call external tools in a structured
way. Once configured, your AI assistant can manage customers, send transfers,
check balances, and more -- all through natural conversation.
...
The MCP server inherits the CLI's two auth/context modes:

- **Consumer / personal** — a bearer token (OAuth device flow); requests send
  `x-blaze-personal: true`. Powers personal-session tools such as
  `blaze_list_connected_payment_methods` and `blaze_withdraw_to_payment_method`.
- **Business** — an API key (`sk_test_` / `sk_live_`) or `--business <id>`;
  requests send `x-business-id`. Powers business tools (customers, transfers,
  payouts, bills, analytics).
...
1. Open Claude Desktop settings and navigate to the MCP servers configuration
   file. On macOS this is typically
   `~/Library/Application Support/Claude/claude_desktop_config.json`. On Windows
   it is `%APPDATA%\Claude\claude_desktop_config.json`.
...
3. Restart Claude Desktop. You should see "blaze" listed as an available MCP
   server with 83 tools.

**Alternative using a global install:**

If you have installed `@blaze-money/cli` globally, you can reference the binary
directly:
...
1. Open Cursor settings and navigate to **MCP Servers** (or edit
...
Alternatively, add the server to your project's `.claude/settings.json` to share
the configuration with your team:
...
Once configured, Claude Code can call any of the 83 Blaze tools directly inside
your terminal. Try asking:
...
Codex will have access to all 83 Blaze tools and can perform payment operations
autonomously.
...
The Blaze MCP server communicates over **stdio** (standard input/output). Any
MCP client that supports the stdio transport can connect.
...
The server reads the API key from the `BLAZE_API_KEY` environment variable or
from `~/.blaze/config.json` (if previously saved with `blaze auth login`).

---

## Available Tools

The MCP server exposes 83 tools organized by resource.

> **Note:** API key management operations are intentionally excluded from the
> MCP server. Team member removal and ownership transfer are also excluded.
...
| `blaze_create_external_account` | Add a bank account or crypto wallet                  | `customer_id`, `type`, `routing_number`, `account_number`, `iban`, `wallet_address`, `network` (one of `stellar`, `ethereum`, `polygon`, `solana`, `base`, `arbitrum`, `optimism`, `avalanche`) |
...
| Tool                                   | Description                                                                          | Key Parameters                                                                            |
| -------------------------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| `blaze_list_withdrawals`               | List withdrawals with optional status filter (business)                              | `limit`, `cursor`, `status`                                                               |
| `blaze_get_withdrawal`                 | Get a single withdrawal by ID (business)                                             | `id`                                                                                      |
| `blaze_create_withdrawal`              | Create a withdrawal to a customer's external account (business)                      | `external_account_id`, `amount`, `currency`, `note`, `metadata`                           |
| `blaze_list_connected_payment_methods` | List YOUR OWN connected methods you can withdraw to (personal session)               | `all`                                                                                     |
| `blaze_withdraw_to_payment_method`     | **IRREVERSIBLE.** Withdraw YOUR OWN balance to YOUR OWN bank/card (personal session) | `payment_method_id`, `amount`, `currency`, `instant_transfer`, `confirm` (must be `true`) |
...
Requires a Plaid-connected bank on the active business and the
`SPENDING_INSIGHTS` feature gate. Read-only — these tools cannot move money.
...
Requires the `BILLS_PAGE` feature gate. Payment execution uses a **two-phase
quote-then-confirm** pattern: call `blaze_quote_bill_payment` first, surface the
quote to the user, then call `blaze_pay_bill` with `quote_id` and
`confirm: true`. Quotes expire after 15 minutes.
...
> **Note:** There is no `blaze_disconnect_gmail` MCP tool — Gmail disconnection
> is CLI-only (`blaze bills gmail-integrations disconnect <id>`) to avoid agents
> silently breaking the bill ingestion pipeline.
...
Once configured, you can interact with Blaze through natural language in your AI
assistant.
...
The assistant calls `blaze_send_money` with `email`, `amount`, and `note`
parameters.
...
The assistant calls `blaze_list_transfers` with `limit: 5` and
`status: "completed"`.
...
The assistant calls `blaze_create_payment_link` with `amount: 50` and
...
1. The `BLAZE_API_KEY` environment variable is set in the MCP server
   configuration.
2. Or, you have previously run `blaze auth login --api-key <key>` to save a key
   to `~/.blaze/config.json`.

### "Authentication failed" (401)

The API key is present but invalid. Double check that the key is correct and has
not been revoked. Test keys start with `sk_test_` and live keys start with
`sk_live_`.

### Tools not appearing in Claude Desktop

- Confirm that `claude_desktop_config.json` has valid JSON (no trailing commas,
  correct quoting).
...
- If using a global install, confirm the `blaze` binary is accessible from the
  shell that the MCP client uses.

### Permission errors (403)

Your API key does not have the required permissions for the requested operation.
Contact your Blaze account administrator to verify key permissions.

### Rate limiting (429)

You are sending too many requests. The AI assistant will see an error message
and can retry after a brief pause. If this happens frequently, consider reducing
the pace of automated workflows.
